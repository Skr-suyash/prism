  ...
  return (
    <section className="bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col">
      ...
      <div className="p-6 flex-1 flex flex-col justify-end">
        <div className="relative h-64 w-full flex items-end justify-between gap-1 mt-24">
          <div className="absolute right-0 -top-10 text-[10px] font-bold text-gray-400 z-10 pointer-events-none">
          ...
          {data.map((d) => {
            ...
            return (
              <div key={d.hour} className="relative flex flex-col items-center flex-1 h-full justify-end group z-10 hover:z-50">

                {/* 1. Added relative to the Bar and moved the Tooltip INSIDE it */}
                <div
                  className={`relative w-full rounded-t-sm transition-all duration-300 ${isPeak
                      ? "bg-rose-400 group-hover:bg-rose-500"
                      : "bg-blue-400 group-hover:bg-blue-500"
                    }`}
                  style={{ height: `${heightPct}%`, minHeight: '4px' }}
                >
                  {/* 2. Tooltip is now positioned using bottom-full and -translate-x-1/2 */}
                  <div
                    className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 flex flex-col items-center bg-gray-900 text-white text-xs px-3 py-2 rounded-lg shadow-xl whitespace-nowrap z-50 pointer-events-none transition-opacity duration-200"
                  >
